...
if (!isset($data['name']) || !isset($data['department']) || !isset($data['email']) || !isset($data['doctor']) || !isset($data['created_at'])) {
...
$insert_query = $conn->prepare("INSERT INTO appointments (user_name, department_name, email, doctor_name, date, status) VALUES (?, ?, ?, ?, ?, ?)");
$insert_query->bind_param("ssssss", $user_name, $department_name, $email, $doctor_name, $date, $status);
